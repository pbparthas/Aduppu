...
Aduppu syncs through **your own Google Drive** — there is no Aduppu server.
...
design. You will never paste a password or secret key into Aduppu.
...
1. Sign in with the Google account whose Drive should hold your recipes and
   open the New Project form directly:
   **https://console.cloud.google.com/projectcreate**
   (If you get a welcome/terms page first, accept and open the link again.)
2. Name: `Aduppu` → **Create** → wait a few seconds, then make sure the new
...
2. Check `Aduppu` is the selected project in the top bar → **Enable**.
...
2. App name: `Aduppu`. User support email: your email. Audience/User type:
...
3. **Important — publish the app:** on the consent screen (or "Audience")
   ...
   *Why:* while an app is in Testing, Google kills its sign-ins every 7 days —
   ...
   Aduppu only uses the `drive.file` scope (it can only see files it created,
...
1. Open **https://console.cloud.google.com/apis/credentials/oauthclient**
   ...
   OAuth client ID).
2. Application type: **Web application**. Name: `Aduppu`.
3. Under **Authorized JavaScript origins**, add every URL you'll open the app
   from:
   - `http://localhost:5173` (local development)
   - `https://aduppu.orionforge.dev` (once the PWA is hosted)
4. **Create**, then copy the **Client ID** — it looks like
   `1234567890-abcdefg.apps.googleusercontent.com`.

## Step 5 — Paste it into Aduppu

Open Aduppu; the first-run screen asks for the Client ID. Paste it, save, and
...
After that, an `Aduppu/` folder appears in your Drive containing your recipes as
