...
//   Aduppu/schema.json                     — data-format version (sync guard)
//   Aduppu/items/<itemId>.json             — one dish, plan, or log per file
//   Aduppu/attachments/<attId>__<name>     — one file per image/attachment
//
// `getToken` is injected (browser: GIS token client).
...
export function createDriveClient({ getToken, baseUrl = 'https://www.googleapis.com' }) {
  ...
  function multipart(metadata, content, mime) {
    const boundary = 'aduppu-' + Math.random().toString(36).slice(2);
    const body = new Blob([
  ...
  return {
    ...
    async ensureSetup() {
      if (folderId) return;
      const folder = await findOne(`name = 'Aduppu' and mimeType = '${FOLDER_MIME}' and trashed = false`);
      folderId = folder ? folder.id : await createFolder('Aduppu');
