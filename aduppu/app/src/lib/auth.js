...
//     by setting AUTH_WORKER below (or localStorage 'ad_auth_worker' to test).
//
// The client ID is public; there is no secret in the browser in either mode.

const SCOPE = 'https://www.googleapis.com/auth/drive.file';

// Point this at the deployed Worker (e.g. 'https://aduppu-auth.orionforge.dev') to
...
function workerUrl() {
  try { return localStorage.getItem('ad_auth_worker') || AUTH_WORKER_DEFAULT; } catch { return AUTH_WORKER_DEFAULT; }
}
...
function createTokenAuth(clientId) {
  ...
  let expiresAt = Number(localStorage.getItem('ad_token_exp')) || 0;
  ...
  function request(prompt) {
    return new Promise((resolve, reject) => {
      tokenClient.callback = (resp) => {
        ...
        localStorage.setItem('ad_token_exp', String(expiresAt));
  ...
  return {
    ...
    isSignedIn: () => localStorage.getItem('ad_signed_in') === '1',
    signOut() {
      if (token) window.google?.accounts.oauth2.revoke(token, () => {});
      token = null;
      expiresAt = 0;
      localStorage.removeItem('ad_token');
      localStorage.removeItem('ad_token_exp');
...
function createBackendAuth(clientId, worker) {
  ...
  async function post(path, body) {
    const res = await fetch(worker + path, {
      method: 'POST',
      credentials: 'include', // send/receive the ad_session cookie
